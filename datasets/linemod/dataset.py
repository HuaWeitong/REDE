...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, root, real, render, fuse, obj_id):
        self.obj_id = obj_id
        ...
        if real:
            if self.mode == 'train':
                input_file = open('{0}/data/{1}/train.txt'.format(self.root, '%02d' % self.obj_id))
            else:
                input_file = open('{0}/data/{1}/test.txt'.format(self.root, '%02d' % self.obj_id))
            while 1:
                item_count += 1
                ...
                self.list_rgb.append('{0}/data/{1}/rgb/{2}.png'.format(self.root, '%02d' % self.obj_id, input_line))
                self.list_depth.append('{0}/data/{1}/depth/{2}.png'.format(self.root, '%02d' % self.obj_id, input_line))
                if self.mode == 'eval':
                    self.list_label.append('{0}/segnet_results/{1}_label/{2}_label.png'.format(self.root, '%02d' % self.obj_id, input_line))
                else:
                    self.list_label.append('{0}/data/{1}/mask/{2}.png'.format(self.root, '%02d' % self.obj_id, input_line))
                self.list_pose.append('{0}/data/{1}/pose/pose{2}.npy'.format(self.root, '%02d' % self.obj_id, str(int(input_line))))

        if render:
            for i in range(1000):
                self.list_rgb.append('{0}/renders/{1}/{2}.jpg'.format(self.root, '%02d' % self.obj_id, i))
                self.list_depth.append('{0}/renders/{1}/{2}_depth.png'.format(self.root, '%02d' % self.obj_id, i))
                self.list_label.append('{0}/renders/{1}/{2}_mask.png'.format(self.root, '%02d' % self.obj_id, i))
